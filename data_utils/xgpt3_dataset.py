...
class MultiModalDataset(Dataset):
    ...
    def __init__(self, input_files, tokenizer, processors,
                 max_completion_length=1536, max_length=2048,
                 media_tokens=['<image>']):
        args = get_args()
        self.dataset = []
        ...
        if args.use_learnable_tokens:
            self.media_lengths = {'<image>': 1+args.num_learnable_tokens}
        else:
            self.media_lengths = {'<image>': 1+257}
        # self.media_lengths = {'<image>': 1+4}
    ...
    def _extract_text_token_from_conversation(self, data, max_length, index):
        ...
        if all([media_token not in conversation for media_token in self.media_tokens.keys()]):
            ...
            # tmp_enc_chunk = enc_chunk.clone()
            # media_indices = torch.where(tmp_enc_chunk < 0)[0].tolist()
            # tmp_enc_chunk[tmp_enc_chunk >= 0] = 1
            # non_media_mask = tmp_enc_chunk.tolist()
            # for idx in media_indices:
            #     # non_media_mask[idx] = [0] * self.media_lengths['<image>']
            #     non_media_mask[idx] = [0]
            # non_media_mask_tmp = []
            # for idx, l in enumerate(non_media_mask):
            #     if type(l) == list:
            #         non_media_mask_tmp.append(torch.tensor(l))
            #     # elif ((isinstance(l, int) and idx == 0) or type(non_media_mask[idx - 1]) == list) and len(prompt_chunk) > 0:
            #     #     non_media_mask_tmp.append(torch.tensor([0]))
            #     else:
            #         non_media_mask_tmp.append(torch.tensor([l]))
            # non_media_mask = torch.cat(non_media_mask_tmp)
            # # non_media_mask = torch.cat([torch.tensor(l) if type(l) == list else torch.tensor([l]) for l in non_media_mask])
            # non_media_mask = non_media_mask[1:].long()
...
def build_train_valid_test_datasets(input_file, tokenizer, max_completion_length=80, max_length=80, config=None, **kwargs):
    ...
    train_ds = MultiModalDataset(
        input_file[0], tokenizer, train_processors, max_completion_length, max_length)
    valid_ds = MultiModalDataset(
        input_file[1], tokenizer, valid_processors, max_completion_length, max_length)
    test_ds = None
